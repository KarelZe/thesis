...
**Semi-supervised GBRTs**

We analyse the robustness of gls-GBRT with self-training on gls-ise data in cref-tab and gls-cboe data in cref-tab. Similar to what we observe for the vanilla gls-GBRT, gls-GBRT with self-training outperforms the respective benchmarks on almost all subsets. The only exceptions are index options and options traded outside the quotes, where the model performs worse than $\operatorname{gsu}_{\mathrm{small}}$ and $\operatorname{gsu}_{\mathrm{large}}$.
