...
- Additionally, they propose contrastive self-supervised pre-training when labelled data is scarce. This makes the architecture suitable for semi-supervised learning.
...
![[saint-architecture.png]]
	...
	- The saint architecture consists of $L$ stages out of which each stage consists of a self-attention transformer block (eq. encoder of [[@vaswaniAttentionAllYou2017]]) and a intersample transformer block (similar to self-attention transformer block, but with intersample attention). Each transformer block uses skip connections, layer normalisation and $\operatorname{GeLU}(\cdot)$. 
...
- **Contrastrive learning:**
	- In contrastive pre-training the distance between two views of the same point is minimised while the distance between two other points is maximised.
