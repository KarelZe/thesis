...
Instead of learning our models on unprocessed data directly, we perform *feature engineering* to convert input data into a form that can be digested by the models and expectedly improves their convergence behaviour. Feature engineering is also crucial for the model's performance, as it helps to unravel relationships between the predictors and the response variable [[@kuhnFeatureEngineeringSelection2020]] (xi). <mark style="background: #FFB8EBA6;">Despite trees and neural networks being flexible estimators, An example for features, that can not be synthesized from the data is the work of [[@heatonEmpiricalAnalysisFeature2016]]. </mark>
