...
- Decoder learns to sample from all possible outputs of the target language. 10 most likely translation etc. Computationally unfeasible. To solve we look at one token at a time. Decoder works auto-regressively. Choose most likely token. and update all the inputs / things we have computed so far.
...
- Uses a query vector ( decoder state) and key vector (all encoder states). For each query-key pair the weight is calculated. Finally, result is normalised using softmax. The values are then combined as the weighted sum.  Keys are weighted by the attention weights.
- Scale of dot product increases with dimension of vectors, thats why scaled dot product normalises by the size of the vector. $$
...
- Idea of multi-headed attention is, to use multiple attention heads, so that the model can focus on different parts of the sentence. Heads are learnt independently.
- Training tricks: layer norm helps to ensure layers remain in a reasonable range, specialised training schedule: adjust default learning  rate of Adam, label smoothing insert uncertainity in the training process, and masking
...
- Layer normalisation is identical to batch correction, except instead of acting on the sample dimension _n,_ LayerNorm normalises the feature dimension _d_.
...
3. Non-linear softmax activation. Exponentially amplifies large values and brings small values to zero. Performs normalisation.
4. New embeddings are contextualised, as each embedding now contains a  fraction of the other embeddings. If a embedding has hardly any relationship to other tokens, its embedding is similar to the input embedding.
...
$$
- Output of the dot product is the attention philtre. Attention philtre contains the attention scores. Scale them and squash them using softmax activation.
- Multiplication with values and attention philtre gives a philtre value matrix which assigns a high focus to features that are more important.
...
- In training model gets provided with source and target dialogue. Target dialogue is masked to come up with own answers.
- Masking operation is a philtre matrix, whereby neg inf is added to all words afterwords. Neg inf become 0 in softmax layer.
