...
Differentiable trees combine properties of neural networks and tree-based learners. In anticipation of cref-[[🎄Decision Trees]], standard decision trees are not differentiable and rely on local, greedy splitting procedures. On the other hand, neural networks, are trained in an end-to-end fashion using back-propagation. Differentiable trees become differentiable by using a soft splitting criterion within nodes of the tree, as proposed in (Hazimeh, Kontschieder, Popov). A frequently benchmarked variant of this family is the NODE network ([[@popovNeuralObliviousDecision2019]]3--5), which uses oblivious decision trees (trees, that use the same splitting criterion for all nodes at a single level) and uses entmax (Peters; function which maps a vector of real-valued scores to a discrete probability distribution) to perform soft splits and combines trees as layers in a neural network. NODE delivers performance on-par with gls-gbt in the author's experiments ([[@popovNeuralObliviousDecision2019]]7). The success of NODE is not corroborated by independent studies. In ([[@kadraWelltunedSimpleNets2021]]7) falls short to achieve the performance of simple, regularised networks. ([[@gorishniyRevisitingDeepLearning2021]]7) and ([[@borisovDeepNeuralNetworks2022]]14) demonstrate, that NODE is inferior to attention-based architectures. Hence, our focus shifts towards attention-based networks to perform trade classification.
...
Boosting, as an ensemble learning framework, is one of the most powerful classification algorithms in supervised learning. Based on the gradient descent view of boosting [Mason et al., 2000], many semi-supervised boosting methods have been proposed, such as SMarginBoost [d’Alch ́ e-Buc et al., 2002], ASSEMBLE [Bennett et al., 2002], RegBoost [Chen and Wang, 2007; Chen and Wang, 2011], SemiBoost [Mallapragada et al., 2009], SERBoost [Saffari et al., 2008] and information theoretic regularisation based boosting [Zheng et al., 2009], where a margin loss function is minimised over both labelled and unlabeled data by the functional gradient descent method. The effectiveness of these methods can be ascribed to their tendency to produce large margin classifiers with a small classification error. However, these algorithms were not designed to directly maximise the margin (although some of them have the effects of margin enforcing), and the objective functions are not related to the margin in the sense that one can minimise these loss functions while simultaneously achieving a bad margin [Rudin et al., 2004]. Therefore, a natural goal is to construct classifiers that directly optimise margins as measured on both labelled and unlabeled data.
...
- Sophisticated neural network architectures might not be required, but rather a mix of regularisation approaches to regularise MLPs [[@kadraWelltunedSimpleNets2021]].
...
“Differentiable trees. The significant drawback of tree-based approaches is that they usually do not allow end-to-end optimisation and employ greedy, local optimisation procedures for tree construction.” (Popov et al., 2019, p. 2)
