...
\begin{figure}[h]
    ...
    \caption[Approximation of Decision Tree]{Approximation of Decision Tree.}
...
After $M$ iterations we obtain the final estimate calculated as $F_M(\mathbf{x})$. To avoid \gls{overfitting} the residuals, only proportional steps towards the negative gradient are taken, which is controlled by the learning rate (...) \autocite[][13]{friedmanGreedyFunctionApproximation2001}. The learning rate (...) and the size of the ensemble $M$ are deeply intertwined are best tuned together \autocite[][13]{friedmanGreedyFunctionApproximation2001}.
...
\begin{figure}[ht]
    ...
    \caption[Positional Encoding of Transformer]{Positional encoding. The encoding is added onto the \gls{token} embeddings to add positional information. The heatmap visualizes the uniquely identifying pattern created from sine and cosine signals at increasing frequencies across the embedding dimension.}
...
\begin{figure}[h]
    ...
    \caption[Decision Boundary of a Supervised and Semi-Supervised Classifier]{Decision Boundary of a Supervised and Semi-Supervised Classifier.}
