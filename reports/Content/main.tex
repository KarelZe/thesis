...
We introduce some more notation, which we will use throughout. Each data instance consists of a feature vector and the target. The former is given by $\boldsymbol{x} \in \mathbb{R}^m$ and described by a random variable $X$. Features in $\boldsymbol{x}$ may be numerical, e.g., the previous trade price or categorical e.g., the option type. Like before, the target or trade initiator is given by $y \in \mathcal{Y}$ and described by a random variable $Y$. Each data instance is sampled from a joint probability distribution $p^*(X, Y)$. The training set with $N$ i.i.d. samples drawn from $p^*$ is denoted by $\mathcal{D}_N=\left\{\left(\boldsymbol{x}_i, y_i\right)\right\}_{i=1}^N$. For convienience, we define a $(n \times m)$-dimensional feature matrix $\boldsymbol{X}=\left[\boldsymbol{x}_1,\cdots, \boldsymbol{x}_N\right]^{\top}$, that stores all instances and a corresponding vector of labels $\boldsymbol{y}=\left[y_1,\cdots, y_N \right]^{\top}$.
...
For a tree with $M$ regions $R_1, R_2,\ldots, R_M$, and some numerical input $\boldsymbol{x}$ the tree can be modelled as:
\begin{equation}
    h(\boldsymbol{x})=\sum_{m=1}^{M} c_{m} \mathbb{I}\left(\boldsymbol{x} \in R_{m}\right),
...
Semi-supervised learning is centred around the \textit{assumption of smoothness}, which states that if two samples $\boldsymbol{x}_{1}$ and $\boldsymbol{x}_{2}$ are nearby in a high-density region, their class labels $y_{1}$ and $y_{2}$ should also be similar. Vice versa, if datapoints are separated by a low-density region, their labels may be different \autocite[][5]{chapelleSemisupervisedLearning2006}. Applied to trade classification, we implicitly assume that trades with similar features, such as a common trade price and quotes, conform to the same class. The purpose of unlabelled trades is to help efficiently determine the boundary around regions of neighbouring trades resulting in an improved classification.

The semi-supervised setting requires to extend our notation from \cref{sec:problem-framing}, by dividing the dataset into labelled and unlabelled instances. Henceforth, $\boldsymbol{X}_{l} = \left[\boldsymbol{x}_{1},\ldots, \boldsymbol{x}_{l}\right]^{\top}$ is used for instances, where labels $\boldsymbol{y}_{l} = \left[y_{1},\ldots, y_{l}\right]^{\top}$ are known and $\boldsymbol{X}_{u} = \left[\boldsymbol{x}_{l+1},\ldots, \boldsymbol{x}_{l+u}\right]^{\top}$ for unlabelled datapoints. Like before, all trades are ordered by datetime.
...
\begin{figure}[ht]
    \centering
    \includegraphics{30sl6vqf-hyperparam-search-space.pdf}
    \caption{Hyperparameter Search Space on \gls{CBOE} Validation Set (Classical)}
    \label{fig:cboe-hyperparam-classical}
\end{figure}
...
\begin{figure}[ht]
    \centering
    \includegraphics{2w28suql-hyperparam-search-space.pdf}
    \caption{Hyperparameter Search Space on \gls{CBOE} Validation Set (Classical Size)}
    \label{fig:cboe-hyperparam-classical-size}
\end{figure}
...
\begin{figure}[ht]
    \centering
    \includegraphics{2qzvvdbw-hyperparam-search-space.pdf}
    \caption{Hyperparameter Search Space on \gls{CBOE} Validation Set (ML)}
    \label{fig:cboe-hyperparam-ml}
\end{figure}
...
The quality of the classifier is better, the higher the share of trades is where the predicted trade initiator equals the true trade initiator. We assess the quality of our model’s prediction in terms of \emph{accuracy}. Formally, accuracy can be stated as:

\begin{equation}
\operatorname{Accuracy} = 1 - \mathcal{L}_{0-1}(\boldsymbol{y}, \hat{\boldsymbol{y}})
\end{equation}

where $\mathcal{L}_{0-1}(\cdot)$ is the 0-1-loss given by:
\begin{equation}
...
Intuitively, from the 0-1-loss we obtain the error rate on the dataset, as for every misclassified trade we count a loss of one and normalize by the number of samples $N$, which gives the normalized 0-1-loss.

Our dataset is balanced and buyer-initiated trades are predicted as sellers --~\textit{false positives}~-- and buyer-initiated trades are predicted as seller-initiated --~\textit{false negatives}~-- have similar associated costs, which makes the accuracy a reasoned choice as a performance metric. As the 0-1-loss and in consequence, the accuracy is not differentiable, we cannot use it in optimization, but use it as an early stopping criterion to halt training or as an optimization target in the hyperparameter search. We report the accuracy of the test set.
