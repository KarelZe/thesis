...
Aside from the feed-forward sublayer, the decoder contains a sublayer for multi-head self-attention on the output of the encoder, known as cross-attention, and and a masked variant of the of the multi-head self-attention layer for use on the output sequence. Causal masking enforces the auto-regressive properties of the decoder.
...
Datasets often comprise not only continuous features but also categorical features like the underlying. In the context of tabular Transformers, learned categorical embeddings are widely used, which are similar to word embeddings % \autocites[][1]{wangTransTabLearningTransferable}
\autocites[][1]{gorishniyRevisitingDeepLearning2021}[][1]{huangTabTransformerTabularData2020}[][1]{somepalliSAINTImprovedNeural2021}. Similar to the word embedding, each category is mapped to an embedding vector using a learned, embedding matrix. Due to the heterogeneous nature of tabular data, embeddings may not be shared between features.
