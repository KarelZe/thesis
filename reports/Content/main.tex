...
The selection of methods in previous works is performed arbitrarily and leaves out advancements in machine learning on tabular data (cp. \cref{sec:related-work}). This thesis provides a succinct discussion and selects a set of supervised classifiers based on empirical evidence. We impose the following requirements a classifier must fulfill:

We impose the following requirements a classifier must fulfill:

\emph{performance:} The approach must deliver state-of-the-art performance in tabular classification tasks.

\emph{scalability:} The approach must scale to datasets with $>$ 10 Mio. samples.
...
\end{equation}
After $M$ iterations we obtain the final estimate calculated as $F_M(\mathbf{x})$. To avoid \gls{overfitting} the residuals, only proportional steps towards the negative gradient are taken, which is controlled by the learning rate $\eta$ \autocite[][13]{friedmanGreedyFunctionApproximation2001}. The learning rate $\eta$ and the size of the ensemble $M$ are deeply intertwined are best tuned together \autocite[][13]{friedmanGreedyFunctionApproximation2001}.
...
\begin{equation}
    \mathbf{S} = \mathbf{S}+\mathbf{W}_{\mathrm{mlp} 2}^l \mathtt{ReLU}\left(\mathbf{W}_{\mathrm{mlp} 1}^l \mathbf{S}+\mathbf{b}_{\mathrm{mlp} 1}^l 1^{\top}\right)+\mathbf{b}_{\mathrm{mlp} 2}^l 1^{\top},
\end{equation}
...
\begin{equation}
    P=\mathtt{linear}\left(\mathtt{ReLU}\left(\mathtt{layer\_norm}\left(\mathbf{S}\left[:,0\right]\right)\right)\right).
    \label{eq:bert-ft}
\end{equation}
...
\begin{figure}[ht]
    ...
    \caption[Random Token Replacement]{Random Token Replacement. Own work inspired by \autocite[][2--3]{clarkElectraPretrainingText2020}.}
...
\begin{table}[H]
    \centering
    \caption{Hyperparameter Search Space of Gradient Boosting}
    \label{tab:hyperparameter-space}
    \begin{tabular}{@{}llSSS@{}}
        \toprule
        Hyerparameter                    & Distribution                              & {FS 1}           & {FS 2}               & {FS 3}            \\ \midrule
...
The search space of the FT-Transformer is given by:

\begin{table}[H]
    \centering
        \caption{Hyperparameter Search Space of FT-Transformer}
        \label{tab:hyperparameter-space-2}
    \begin{tabular}{@{}llSSS@{}}
    \toprule
    Hyperparameter         & Distribution  & {FS 1} & {FS 2} & {FS 3} \\ \midrule
    Layers $L$               & $\operatorname{UniformInt}[1,6]$ &      &      &      \\
    $d_{\mathrm{e}}$             & $\operatorname{UniformInt}[64, 256]$                                 &      &      &      \\
    Attention dropout & $\operatorname{Uniform}[0, 0.05]$                                 &      &      &      \\
    \gls{FFN} dropout       & $\operatorname{Uniform}[0, 0.05]$                                 &      &      &      \\
    Learning rate $\eta$ &  $\operatorname{LogUniform}[\num{3e-5}, \num{3e-4}]$                                &      &      &      \\ 
    weight decay     & $\operatorname{LogUniform}[\num{1e-6}, \num{1e-3}]$                                 &      &      &      \\ \bottomrule
    \end{tabular}
\end{table}
    
The hyperparameter search for the FT-Transformer is almost identical to \textcite[][18]{gorishniyRevisitingDeepLearning2021} (variant (b)). We halfen the maximum dimension to facilitate training on large feature sets.
