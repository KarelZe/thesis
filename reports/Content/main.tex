...
\todo{verify formula. What happens with unclassified trades?}
...
\todo{Add end of discussion to bebinning and make sure the why is clear.}
...
\todo{Explain why logloss is so high}
\begin{figure}[ht]
    \centering
...
\todo{Put parameters into footnote. Unimportant detail.}
...
\todo{Rotate page with 2 images, side by side}
...
\end{figure}
\todo{fix (a) counter in subfigure}
...
\emph{Gradient Boosting With Self-Training}
\todo{Do not visualize search space, rather document results table and provide links to experiments in footnotes.}
\begin{figure}[!b]
...
\table{Move table to here.}
\emph{Transformer}
\todo{Explain Transformer results}
(insert here)

Results are tabulated in \cref{tab:solutions-transformer}.

\emph{Transformer With Pre-Training}
\todo{add tabulated results here. }
(insert here)
...
\todo{Rewrite to hyperparameter space, report found solution, report grauer solution, report validation accuracies}
\begin{table}[H]
    \centering
...
\end{table}
\todo{link to concrete experiment}
...
\todo{Pick up in text, that  the combination is the one we previously referred to as the GSU method. Say it is more difficult benchmark, than my found solution}
\todo{check if best combination is quote(nbbo) quote(ex) rev-tick}
...
\todo{discuss why not report log loss / cross entropy loss. see notes!}
