...
Thus, we frame trade signing as a probabilistic classification problem. This is similar to the work of a \textcite[][272]{easleyDiscerningInformationTrade2016}, who retrieve probabilities from \emph{bulked} versions of the tick rule and \gls{BVC} algorithm, but on a \emph{trade-per-trade} basis.

Vice versa, for probability estimate $\hat{p}$ we can retrieve the most probable class in $\mathbb{Y}$ as:
\begin{equation}
    \hat{y}=\arg\max_{y \in \mathbb{Y}} \hat{p}(y).
    \label{eq:arg-max-p}
\end{equation}
