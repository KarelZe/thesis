...
\begin{table}[!h]
    ...
    \begin{tabular}{@{}llSSS@{}}
        \toprule
        Hyperparameter                       & Distribution                                        & {\glsentryshort{FS} Classical}                   & {\glsentryshort{FS} Size}                   & {\glsentryshort{FS} Option}                   \\ \midrule
        Layers $L$                           & $\operatorname{UniformInt}[1,6]$                    &  4                        &                          &                          \\
        Embedding dimension $d_{\mathrm{e}}$ & $\operatorname{UniformInt}[64, 256]$                &  248                        &                          &                          \\
        Attention dropout                    & $\operatorname{Uniform}[0, 0.5]$                    &  0.04424625102595975                        &                          &                          \\
        \gls{FFN} dropout                    & $\operatorname{Uniform}[0, 0.5]$                    &  0.0979914312095726                        &                          &                          \\
        Learning rate $\eta$                 & $\operatorname{LogUniform}[\num{3e-5}, \num{3e-4}]$ &  \num{1e-6}                        &                          &                          \\
        weight decay $\lambda$               & $\operatorname{LogUniform}[\num{1e-6}, \num{1e-3}]$ &  \num{6e-5}                         &                          &                          \\ \midrule
        Validation Accuracy in \%            &                                                     & {$\uparrow \num{65.13623935106421}$} & {$\downarrow \num{0.0}$} & {$\downarrow \num{0.0}$} \\ \bottomrule
    \end{tabular}
\end{table}
