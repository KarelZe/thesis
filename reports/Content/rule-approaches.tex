...
\todo{There are two opposing arguments about the relationship between the probability of successful classification and trade distance. One suggests that classification errors are a result of asynchronous trade and quote times and the accuracy of trade classification rules should increase when the trade distance is larger. The second argument is based on the possibility that the tick rule could become less accurate the longer the time between trades (Finucane, 2000). Consistent with the first argument, the marginal effects are positive for all four regressions, indicating that larger the trade distances increase the probability of correct classification. We also examine the time between the prevailing quote and the last quote update - quote distance. Short time intervals between quote changes can lead to the problem of asynchronous prices and hence decrease classification accuracy. The results of our regressions confirm that higher quote distances leads to better classification.9 (Found in chakrabarty paper)}
...
The performance of the tick rules hinges on the availability of recent trade prices for inference. For infrequently traded assets this can pose a problem, as outdated prices might loose their relevancy in classification, as documented in \textcite[][568]{finucaneDirectTestMethods2000}.
