...
its vocabulary $C_m$ with $N_{C_m}$ categories. Assume, the $m$-th feature is categorical. The specific embeddings $\gls{e}$ are queried with a unique integer key $c_{m} \in C_m \cong\left[N_{C_m}\right]$ from the learned embedding matrix $\mathbf{W}_m \in \mathbb{R}^{d_e \times N_{C_m}}$. Finally, a feature-specific bias term $\mathbf{b}_m \in \mathbb{R}$ is added as shown in \cref{eq:categorical-embeddings}. As for the word case, all embeddings of an instance are gathered in $\mathbf{S}$.
\begin{equation}
...
\begin{equation}
    \gls{e}=\gls{W-e}\left[:, \mathbf{s}[t]\right]+\gls{W-p}\left[:, t\right].
