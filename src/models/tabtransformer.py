...
from src.models.basemodel_torch import BaseModelTorch
...
class TabTransformer(BaseModelTorch):
    ...
    def predict_helper(self, X):
        ...
        with torch.no_grad():
            for batch_X in test_loader:
                x_categ = (
                    batch_X[0][:, self.args.cat_idx].int().to(self.device)
                    if self.args.cat_idx
                    else None
                )
    ...
    def attribute(self, X: np.ndarray, y: np.ndarray, strategy=""):
        ...
        Two strategies are supported: default ("") or "diag". The default strategie 
        takes the sum
        over a column of the attention map, while "diag" returns only the diagonal 
        (feature attention to itself)
        of the attention map.
        return array with the same shape as X. The number of columns is equal to the
         number of categorical values in X.
        """
        X = np.array(X, dtype=np.float)
