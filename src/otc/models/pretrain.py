...
from otc.models.permutations import gen_permutations_class
...
class Config:
    ...
    def __post_init__(self):
        ...
        if self.model.positional_encoding:
            self.model.positional_encoding = lib.PeriodicEmbeddingOptions(
...
gen_permutations = gen_permutations_class(
    C.pretrain.replace_strategy, X_num, X_cat, Y, D
)

# apply permutations to train, val, test set
...
# generate masks for train, val, test set
masks_num = {
    part: gen_masks(X_num[part], permutations_num[part])
    for part in ["train", "val", "test"]
}

if X_cat:
    masks_cat = {
        part: gen_masks(X_cat[part], permutations_cat[part])
        for part in ["train", "val", "test"]
    }


# TODO: Seems to construct two layer mlp with ReLU activation in between.
class CLSHead(nn.Module):
    "2 Layer MLP projection head"

    def __init__(self, *, d_in: int, d_hidden: int, use_target: False):
        super().__init__()
...
class PretrainModel(nn.Module):
    ...
    def forward(self, x_num, x_cat, y=None):
        ...
        x = torch.cat(
            [
                x_.flatten(1, 2) if x_.ndim == 3 and self.backbone.FLAT else x_
                for x_ in inputs
                if x_ is not None
            ],
            dim=1,
        ...
        # set backbone and add classification head
        h = self.backbone(x)
        return self.head(h)
...
stream = zero.Stream(
    zero.data.IndexLoader(
        D.size("train"), C.training.batch_size, True, device=device, drop_last=True
    )
)
...
def evaluate(parts):
    ...
    for part in parts:
        ...
        # calculate bce loss
        features_accuracy = (hard_predictions.bool() == masks).sum(
            0
        ) / hard_predictions.shape[0]
        metrics[part] = {
            "pretrain_loss": loss_fn(predictions, masks).item(),
...
def finetune():
    ...
    with tempfile.TemporaryDirectory() as dir_:
        ...
        # load checkpoint of wrapped model. Must be loaded before fine-tuning
...
# set binary cross entropy loss with logits
...
for batch_idx in tqdm(stream_iterations(C.pretrain.n_iterations)):
    ...
    if stream.iteration % C.pretrain.validate_every == 0:
        ...
        if C.pretrain.early_stop_type == "finetune":
            ...
            report.setdefault("metrics", OrderedDict()).setdefault(
                "iteration_scores", OrderedDict()
            )[stream.iteration] = {
                part: {
            ...
            training_log.setdefault("finetune_logs", OrderedDict())[
                stream.iteration
            ] = finetune_logs
